...
int main( int argc, char **argv )
{
  ...
  TFile *fout = new TFile(Form("./process/1-WGCalib/WGcalib_%d.root", runnumber), "RECREATE");
  ...
  //for(int ispill=0; ispill<100; ispill++)
	...
	{
	  auto thespillit 
	    = find(wgwmdata->wgwmspill[divide][idif].begin(), wgwmdata->wgwmspill[idif].end(), thespill);
	  cout << "spill= " << ispill << " "  << *thespillit << " "  << thewgwmspill[idif] << '\n';

...
void WMdata::ReadTree(TString filename, int dif)
{
  difid = dif;
...
bool WMdata::SignalCreation(int ientry, int dif, WGRecon* wgrecon)
{
		...
		for(int ich=0; ich<32; ich++)
		  {
			...
			fdimension->get_pos_wm_FC(dif, thechip, thechan, &posx, &posy, &posz);
			cout << posx << " " << posy << " " << posz << '\n'; 
#endif
		      }
...
bool WMdata::ModeCheck(int spill_mode)
{
...
bool WMdata::BCIDCheck(int bcid)
{
...
int WMdata::GetTEntry(int dif)
{
  difid=dif;
  return wgtree[difid]->GetEntries();
}

int WMdata::GetBunch(int bcid)
{
  int bunch;
  if(bcid<24 || bcid>31)
    bunch=-1;

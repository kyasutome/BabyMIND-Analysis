...
int main ()
{
   ...
   int temp=firstModule;
    ...
    cout << "Number of spills " << minEn << endl;
      ...
      //
     //NumberofEventsMonitor = min (NumberofEventsMonitor,100000);
     ...
     TH2F *SingleEvent[(int)minEn][NumberofEventsMonitor];
     TH1F *SingleEventTime[(int)minEn][NumberofEventsMonitor];
     TH3F *SingleEvent3D[(int)minEn][NumberofEventsMonitor];
     
     
     bool SpillMised = false;
for (Int_t SpillNumber = 0 ; SpillNumber < minEn ; SpillNumber++){ 
    ...
    if (!SpillMised){
     ...
     for (Int_t ih=0; ih < NumberofEventsMonitor;ih++){
        ...
        sEvent3D =  sEvent + "_3D";
        SingleEvent[SpillNumber][ih] = new TH2F(sEvent.c_str(),sEvent.c_str(),  198, -2200, 5720, 95, -986.2, 986.2);
        SingleEventTime[SpillNumber][ih] = new TH1F(sEventTime.c_str(),sEventTime.c_str(), 4096, 0, 4096);
        
        SingleEvent3D[SpillNumber][ih] = new TH3F(sEvent3D.c_str(),sEvent3D.c_str(),  198, -2200, 5720, 16, -1420.5, 1420.5, 95,  -986.2, 986.2);
     }
                              ...
                              if (FEB[MapCon[1][mod]].hitsChannel->at(CoinsLeft) == FEB[MapCon[2][mod]].hitsChannel->at(CoinsRight)){
                                  if (FEB[MapCon[1][mod]].hitAmpl->at(CoinsLeft) > 500 && FEB[MapCon[2][mod]].hitAmpl->at(CoinsRight)>500) {
                                      if (eventNumber<NumberofEventsMonitor){
                                        SingleEvent[SpillNumber][eventNumber]->Fill(ModPos[3][mod],geomHor[4][(int)FEB[MapCon[2][mod]].hitsChannel->at(CoinsRight)],1) ; 
                                        SingleEventTime[SpillNumber][eventNumber] -> Fill(FEB[MapCon[1][mod]].hitLeadTime->at(CoinsLeft));
                                        SingleEventTime[SpillNumber][eventNumber] -> Fill(FEB[MapCon[2][mod]].hitLeadTime->at(CoinsRight));
                                                ...
                                                if ((int)(FEB[MapCon[3][mod]].hitsChannel->at(CoinsTop2)) == (int)(FEB[MapCon[3][mod]].hitsChannel->at(CoinsTop))){
                                                    if (FEB[MapCon[3][mod]].hitsChannel->at(CoinsTop)>=32*MapCon[4][mod] && FEB[MapCon[3][mod]].hitsChannel->at(CoinsTop)<32*(1+MapCon[4][mod])){
                                                        if (FEB[MapCon[3][mod]].hitAmpl->at(CoinsTop) > 200){
                                                            SingleEventTime[SpillNumber][eventNumber] -> Fill(FEB[MapCon[3][mod]].hitLeadTime->at(CoinsTop));
                                                            if(mod == 2 || mod == 4 || mod == 14 || mod == 16 || mod == 18){
                                                                SingleEvent3D[SpillNumber][eventNumber]->Fill(ModPos[3][mod],-geomVer[4][(int)(FEB[MapCon[3][mod]].hitsChannel->at(CoinsTop))%32],geomHor[4][(int)FEB[MapCon[2][mod]].hitsChannel->at(CoinsRight)],1);
                                                            }
                                                            else{
                                                                SingleEvent3D[SpillNumber][eventNumber]->Fill(ModPos[3][mod],geomVer[4][(int)(FEB[MapCon[3][mod]].hitsChannel->at(CoinsTop))%32],geomHor[4][(int)FEB[MapCon[2][mod]].hitsChannel->at(CoinsRight)],1);
                                                            }
     ...
     eventsD->cd();
     for (Int_t ih=0; ih < NumberofEventsMonitor;ih++){
         if (SingleEvent[SpillNumber][ih]->GetEntries()>1) {
            SingleEvent[SpillNumber][ih]->GetYaxis()->SetTitle("vertical position [mm]");
            SingleEvent[SpillNumber][ih]->GetXaxis()->SetTitle("horizontal position [mm]");
            SingleEvent[SpillNumber][ih]->Write();
            delete SingleEvent[SpillNumber][ih];
         }
     }
     /*
     events3D->cd();
     for (Int_t ih=0; ih < NumberofEventsMonitor;ih++){
         if (SingleEvent3D[SpillNumber][ih]->GetEntries()>1) {
            SingleEvent3D[SpillNumber][ih]->GetYaxis()->SetTitle("X position [mm]");
            SingleEvent3D[SpillNumber][ih]->GetXaxis()->SetTitle("Z position [mm]");  
            SingleEvent3D[SpillNumber][ih]->GetZaxis()->SetTitle("Y position [mm]");  
            SingleEvent3D[SpillNumber][ih]->Write();
            delete SingleEvent3D[SpillNumber][ih];
         }
     }
     ...
     for (Int_t ih=0; ih < NumberofEventsMonitor;ih++){
         if (SingleEventTime[SpillNumber][ih]->GetEntries()>1) {
            SingleEventTime[SpillNumber][ih]->GetYaxis()->SetTitle("Number of events");
            SingleEventTime[SpillNumber][ih]->GetXaxis()->SetTitle("Hit time [2.5ns]");
            SingleEventTime[SpillNumber][ih]->Write();
            delete SingleEventTime[SpillNumber][ih];
         }
     }

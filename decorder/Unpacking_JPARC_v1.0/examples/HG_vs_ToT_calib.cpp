...
int main ()
{
    ...
    for (Int_t ih=0; ih<NumberOfFEB; ih++) {
        ...
        if ((TTree*)FileInput->Get(sFEB.c_str())){
            ...
            std::cout << "Number of events = " <<nentries[ih]<<std::endl;
        }
  }
  ...
  Double_t fitPar[5];
  TF1 *f = new TF1("f",bestfit,5,22,5);
  ...
  for (int ih = 0; ih<FEBnumbers.size();ih++) {
        ...
        for (int ik = 0; ik < nentries[FEBnumbers.at(ih)] - 1; ik++){
        
            for (Int_t i = 0; i<FEB[FEBnumbers.at(ih)].hitTimeDif->size(); i++){
                if (FEB[FEBnumbers.at(ih)].hitAmpl->at(i)>0 && FEB[FEBnumbers.at(ih)].hitTimeDif->at(i)>0 && FEB[FEBnumbers.at(ih)].hitAmpl->at(i) < 3500) {
                    //cout<< FEB[FEBnumbers.at(ih)].hitAmpl->at(i)<< " "<< FEB[FEBnumbers.at(ih)].hitLGAmpl->at(i)<<endl;
                    //f (FEB[FEBnumbers.at(ih)].hitTimeDif->at(i)>12){
                    HG_vs_ToT[FEBnumbers.at(ih)][(int)FEB[FEBnumbers.at(ih)].hitsChannel->at(i)]-> Fill(FEB[FEBnumbers.at(ih)].hitTimeDif->at(i),FEB[FEBnumbers.at(ih)].hitAmpl->at(i) , 1);
                    /*} else {
            ...
            free (FEB[FEBnumbers.at(ih)].FEBSN);
            free (FEB[FEBnumbers.at(ih)].SpillNum);
            free (FEB[FEBnumbers.at(ih)].hitsChannel);
            free (FEB[FEBnumbers.at(ih)].hitAmpl);
            free (FEB[FEBnumbers.at(ih)].hitLeadTime);
            free (FEB[FEBnumbers.at(ih)].GTrigTag);
            free (FEB[FEBnumbers.at(ih)].GTrigTime);
            free (FEB[FEBnumbers.at(ih)].hitLGAmpl);
            free (FEB[FEBnumbers.at(ih)].hitTrailTime);
            free (FEB[FEBnumbers.at(ih)].hitTimeDif);
            
            FEB[FEBnumbers.at(ih)].FEBSN=0;
            FEB[FEBnumbers.at(ih)].SpillNum=0;
            FEB[FEBnumbers.at(ih)].hitsChannel=0;
            FEB[FEBnumbers.at(ih)].hitAmpl=0;
            FEB[FEBnumbers.at(ih)].hitLeadTime=0;
            FEB[FEBnumbers.at(ih)].GTrigTag=0;
            FEB[FEBnumbers.at(ih)].GTrigTime=0;
            FEB[FEBnumbers.at(ih)].hitLGAmpl=0;
            FEB[FEBnumbers.at(ih)].hitTrailTime=0;
            FEB[FEBnumbers.at(ih)].hitTimeDif=0;

            FEBtree[FEBnumbers.at(ih)]->GetEntry(ik+1);
        }

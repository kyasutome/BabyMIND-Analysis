...
int main ()
{
    ...
    for (Int_t DecSpillNumber = 1 ; DecSpillNumber < minEn - 1 ; DecSpillNumber+=SpillInFile){
      ...
      TH1F *LostEnergySaba = new TH1F("Energy_deposit_Saba","Energy_deposit_Saba",  440, -2200, 2200);
                ...
                if (mod == 0){
                    ...
                    for (Int_t CoinsAIDA0 = AIDAindex0[0]; CoinsAIDA0 < AIDAindex0[1]; CoinsAIDA0++){
                        for (Int_t CoinsAIDA8 = AIDAindex8[0]; CoinsAIDA8 < AIDAindex8[1]; CoinsAIDA8++){
                            if ( abs(FEB[0].hitTimefromSpill->at(CoinsAIDA0) - FEB[8].hitTimefromSpill->at(CoinsAIDA8)) < 20 && FEB[0].hitCharge_pe->at(CoinsAIDA0)>5 && FEB[8].hitCharge_pe->at(CoinsAIDA8)>5){
                        ...
                        for (Int_t CoinsLeft = leftindex[0]; CoinsLeft < leftindex[1]; CoinsLeft++){
                          for (Int_t CoinsRight = rightindex[0]; CoinsRight < rightindex[1]; CoinsRight++){
                              if (FEB[MapCon[1][mod]].hitsChannel->at(CoinsLeft) == FEB[MapCon[2][mod]].hitsChannel->at(CoinsRight) && 
                                  ...
                                  if (FEB[MapCon[1][mod]].hitCharge_pe->at(CoinsLeft) > 5 && FEB[MapCon[2][mod]].hitCharge_pe->at(CoinsRight)>5) {
                                      if (eventNumber<NumberofEventsMonitor){
                                            ...
                                            for (Int_t CoinsTop2 = CoinsTop; CoinsTop2 < topindex[1]; CoinsTop2++) {
                                                if ((int)(FEB[MapCon[3][mod]].hitsChannel->at(CoinsTop2)) == (int)(FEB[MapCon[3][mod]].hitsChannel->at(CoinsTop))){
                                                    if (FEB[MapCon[3][mod]].hitsChannel->at(CoinsTop)>=32*MapCon[4][mod] && FEB[MapCon[3][mod]].hitsChannel->at(CoinsTop)<32*(1+MapCon[4][mod])){
                                                        if (FEB[MapCon[3][mod]].hitCharge_pe->at(CoinsTop) > 5){
        ...
        for (Int_t ih=0; ih < NumberofEventsMonitor;ih++){
            if (SingleEvent[ih]->GetEntries()>5 ) {
                ...
                if (NumberOfModEx>3){
                    ...
                    int ChargeId[18];
                    std::fill(ChargeId, ChargeId + 18, 0);
                    int MagneticField[18];
                    std::fill(MagneticField, MagneticField + 18, 0);
                    MagneticField[0]=1;
//                     int firstModChargeID[2]={0,0};
                       ...
                       // f[th] = new TF1(sFit.c_str(),"pol1",ModPos[1][th],ModPos[2][th+1]);
                        ...
                        if (AllModulesExist[th][ih]==1){
                            if (AllModulesExist[th+1][ih]==1) {
                                ...
                                if (abs(par[2*th+1]*ModPos[3][th] + par[2*th]) <= 500)
                                    MagneticField[th] = 1 ;
                                else  MagneticField[th] = -1 ;
...
//                                 }
                                ...
                                for (int nextMod = 1; nextMod + th <18; nextMod++ ){
                                    if (AllModulesExist[nextMod + th][ih]==1) {
                                        ...
                                        SingleEvent[ih]->Fit(f[th],"MQWR+");
                                        f[th]->GetParameters(&par[2*th]);
                                        for (int EnDepMod = 0; th - EnDepMod > fitStart; EnDepMod++){
                                            f[th]->GetParameters(&par[2*(th - EnDepMod)]);
                                            if (abs(par[2*(th - EnDepMod)+1]*ModPos[3][th] + par[2*(th - EnDepMod)]) >= 500)
                                                MagneticField[th - EnDepMod] = 1 ;
                                            else  MagneticField[th - EnDepMod] = -1 ;
                                        }
                                        th--;
                                        break;
                                    }
                    ...
                    //cout <<"Event Number: "<< ih<<endl;
                    for ( int th=0; th<LastFiredMod ;th++ ){
                        if ( th != 0 && par[2*th+1] != 0 && par[2*th+3] !=0 ){
                            if ( par[2*th+1] < par[2*th+3]   ){
                                ChargeId[th] = 1;
                            } else if (par[2*th+1] > par[2*th+3] ){
                                ChargeId[th] = -1;
                            } else if ( par[2*th+1] == par[2*th+3] ){
                                ChargeId[th] = ChargeId[th-1];
                            } else 
                                ChargeId[th] =0;
                        }
                        //cout<< par[2*th+1]<< " "<<par[2*th+3]<< " "<<ChargeId[th]<< " "<<endl;
                    }
                    //cout << endl;
                    ...
                    //cout << "Fitting done ";
                    for (int th=1; th<LastFiredMod;th++){
                        if(par[2*th] != 0 || par[2*th+1] != 0){
                            ReconLostEnergy += (EnDep[3][th]) * sqrt( 1 + par[2*th+1]*par[2*th+1]);
                            //ReconLostEnergy += (EnDep[3][th]);
                            //cout << EnDep[3][th]<<endl; 
                            //cout << th << ": " << par[2*th] << " "<< par[2*th+1] << " " << EnDep[3][th] << endl;
                            //FirstModRecon = th;
                        } else {
                            //cout << 0 << endl;
                            for (int ith=0; th - ith > 0; ith++){
                                if(par[2*(th - ith)] != 0 || par[2*(th - ith)+1] != 0){
                                    //cout << th << ": " << par[2*(th - ith)] << " "<< par[2*(th - ith)+1]<< " " << EnDep[3][th] << endl;
                                    ReconLostEnergy += (EnDep[3][th]) * sqrt( 1 + par[2*th+1]*par[2*th+1]);
                                    //cout << EnDep[3][th]<<endl;
                                    break;
                                    return 0;
                    ...
                    ReconLostEnergy += (EnDep[3][LastFiredMod]) * sqrt( 1 + par[2*LastFiredMod-1]*par[2*LastFiredMod-1]) + EnDep[3][0];
                    //ReconLostEnergy += (EnDep[3][LastFiredMod]) + EnDep[3][0];
                    //cout << LastFiredMod << ": " << par[2*LastFiredMod-2] << " "<< par[2*LastFiredMod-1] << " " << EnDep[3][LastFiredMod] << endl;
//                     if (FirsModule[1]!=0){
//                         for(int depmod=0; depmod < FirsModule[1]; depmod++){
//                             ReconLostEnergy+=EnDep[3][depmod];
//                             cout <<"1111:" <<EnDep[3][depmod]<<endl;
//                         }
//                     }
                    int ReconCharge = 0;
                    for (int th=2; th<LastFiredMod -1;th++){
                        if (ChargeId[th] == ChargeId[1] || ChargeId[th]==0) {
                            ReconCharge= ChargeId[1];
                        }
                        else{
                            ReconCharge = 0;
                        }
                    ...
                    if (ReconCharge!=0){ 
                        LostEnergy->Fill(ReconCharge*ReconLostEnergy,1);
                        LostEnergySaba->Fill(ReconCharge*EnDep[2][LastFiredMod],1);
                    }
                }
            }
     ...
     LostEnergy->Write();
     LostEnergySaba->Write();
     delete LostEnergy;
     delete LostEnergySaba;

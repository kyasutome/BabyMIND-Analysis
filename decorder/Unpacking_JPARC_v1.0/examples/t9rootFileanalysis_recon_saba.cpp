...
int main ()
{
    ...
    temp = 0;
    while (!EnergyDep.eof()){
        EnergyDep >> temp >> EnDep[0][temp] >>EnDep[1][temp] >>EnDep[2][temp] >>EnDep[3][temp];
        //cout << temp << " " <<EnDep[0][temp]<< " "<< EnDep[1][temp]<< " " << EnDep[2][temp]<< " " << EnDep[3][temp] << endl;
    }
    EnergyDep.close();
        ...
        for (Int_t ih=0; ih < NumberofEventsMonitor;ih++){
            if (SingleEvent[ih]->GetEntries()>5 ) {
                ...
                if (NumberOfModEx>3){
                    ...
                    LostEnergy->Fill(EnDep[2][LastFiredMod],1);

...
std::vector<double> GetRealGTrig(int i, vector<double> GTrigCheck[i]){
    std::vector<double> v;
...
int main ()
{
    ...
    TDirectory *PerHitsFebRunWithHG = wfile.mkdir("Persent Of Events at FEB with HG per Spill");
    ...
    TH1I *TotallNumberOfEvents = new TH1I( "Totall number of events per FEB", "Totall number of events per FEB",NumberOfFEB,0,NumberOfFEB);
    ...
    TH1F *PersentOfEventsRunWithHG = new TH1F( "Persent of events with HG at FEB", "Persent of events with HG per FEB",NumberOfFEB,0,NumberOfFEB);
    PersentOfEventsRunWithHG->GetYaxis()->SetTitle("%");
    PersentOfEventsRunWithHG->GetXaxis()->SetTitle("FEB");
    PersentOfEventsRunWithHG->SetOption("hist");
    ...
    for (Int_t iFEB=0; iFEB < FEBnumbers.size();iFEB++){
        ...
        PercentHitsFebRunWithHG[FEBnumbers.at(iFEB)] = new TH1D(("FEB_" + sFEB +" Persent of hits with HG per FEB per Spill").c_str(), ("FEB_" + sFEB +" Persent Of Events with HG at FEB per Spill at run").c_str(),30, 0, 100);
        ...
        NumOfHitsPerFEB[FEBnumbers.at(iFEB)] = new TH1I(("FEB_" + sFEB +"Totall number of hits at Channel").c_str(),
            ("FEB_" + sFEB +"Totall number of hits at Channel").c_str(),1001,0,100000);
        NumOfHitsPerFEBWithHG[FEBnumbers.at(iFEB)] = new TH1I(("FEB_" + sFEB +"Totall number of hits with HG at Channel").c_str(),
            ("FEB_" + sFEB +"Totall number of hits with HG at Channel").c_str(),1001,0,100000);
        ...
        TF1 *fitPercentHitsFebRunWithHG = new TF1("fitPercentHitsFebRunWithHG","gaus",0, 50);
        ...
        PersentOfEventsRunWithHG->Fill(FEBnumbers.at(iFEB),fitParPercentHitsFebRunWithHG[1]);
        PercentHitsFebRunWithHG[FEBnumbers.at(iFEB)]->GetYaxis()->SetTitle("Number of entries");
        PercentHitsFebRunWithHG[FEBnumbers.at(iFEB)]->GetXaxis()->SetTitle("Persent of events with HG per Spill at FEB");
        ...
        HitTimefromSpillStart[FEBnumbers.at(iFEB)]->GetXaxis()->SetTitle("Hit Time from Spill Start");
        ...
        HitTimefromSpillStartHG[FEBnumbers.at(iFEB)]->GetXaxis()->SetTitle("Hit Time from Spill Start");
        ...
        for (int ch=0;ch<96;ch++){
            ...
            if (TotallNumOfHitsCh > 4*AvNumEventsRun.at(iFEB)) {
                cout <<"Noisy channel: FEB_"<< FEBnumbers.at(iFEB)<< " channel " <<ch<< " Totall Number of events "<<TotallNumOfHitsCh<<endl;
                oSummaryText.str("");
                oSummaryText<< "FEB_"<< FEBnumbers.at(iFEB)<< " channel " <<ch;
                SummaryTextNoisy.push_back(oSummaryText.str());
            } else if ((TotallNumOfHitsCh < 0.25 * AvNumEventsRun.at(iFEB)) && ChannelExist(FEBnumbers.at(iFEB),ch)) {
                cout <<"Dead channel: FEB_"<< FEBnumbers.at(iFEB)<< " channel " <<ch<< " Totall Number of events "<<TotallNumOfHitsCh<<endl;
                oSummaryText.str("");
                oSummaryText<< "FEB_"<< FEBnumbers.at(iFEB)<< " channel " <<ch;
    ...
    //c->cd(2);
    TPaveText *ptD = new TPaveText(.5,.01,0.99,.8);
    ...
    PersentOfEventsRunWithHG->Write();
    PersentOfEventsRunWithHG->Delete();

...
//                     cout << dw.GetChannelId() << " " << dw.GetAmplitude()<<endl;
          ...
          case MDdataWordBM::HoldTime:
            if (dw.GetHoldTimeID() == 0){
              _holdTimeStart = dw.GetHoldTime();
              //cout << "Board ID " << dw.GetHoldTimeBoardID() << " L1HoldStart Time: " << dw.GetHoldTime()<<endl;
              unsigned int * ptr2 = ptr;
              ptr++;
              dw.SetDataPtr(ptr);
              int gTrig = 0;
              while (dw.GetDataType() != MDdataWordBM::HoldTime){
                ptr++;
                dw.SetDataPtr(ptr);
                if (dw.GetDataType() == MDdataWordBM::TrigHeader){
                  ++gTrig;
                }
                //cout<<dw.GetDataType()<<endl;
                if (dw.GetHoldTimeID() == 1){
                  _holdTimeStop = dw.GetHoldTime() +gTrig*1000;
                } 
              }
              ptr = ptr2;
              dw.SetDataPtr(ptr);
            ...
            // if (dw.GetHoldTimeID() == 1){
            //   _holdTimeStop = dw.GetHoldTime();
            //   //cout << "Board ID " << dw.GetHoldTimeBoardID() << " L1HoldStop Time: " << dw.GetHoldTime()<<endl;
            // }
            ++_nDataWords;
            break;

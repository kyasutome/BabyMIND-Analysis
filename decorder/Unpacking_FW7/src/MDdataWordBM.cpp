...
uint32_t MDdataWordBM::GetHoldTime() {
    if (IsValid())  return ( (*(uint32_t*)(_data) &  HoldTimeMask ) >>  HoldTimeShift );
    return 0;
}
...
uint32_t MDdataWordBM::GetHoldTimeBoardID() {
    if (IsValid())  return ( (*(uint32_t*)(_data) &  HoldTimeBoardIdMask ) >>  HoldTimeBoardIdShift );
    return 0;
}
...
ostream & operator<<(ostream &s, MDdataWordBM &dw) {
  ...
  case MDdataWordBM::HoldTime:
    if (dw.GetHoldTimeID() == 0)
      s << "Board ID " << dw.GetHoldTimeBoardID() << " L1HoldStart Time: " << dw.GetHoldTime();
    if (dw.GetHoldTimeID() == 1)
      s << "Board ID " << dw.GetHoldTimeBoardID() << " L1HoldStop Time: " << dw.GetHoldTime();
    break;

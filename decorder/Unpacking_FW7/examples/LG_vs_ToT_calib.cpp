...
Double_t bestfit(Double_t *x, Double_t *par) {
    ...
    //return par[0]*TMath::Power(x[0],par[1]);
        return par[0]*x[0] + par[1]*x[0]*x[0] + par[2]* x[0]*x[0]*x[0]*x[0] + par[3]* x[0]*x[0]*x[0]*x[0]*x[0]*x[0];
   }
  ...
  for (int ih = 0; ih<FEBnumbers.size();ih++) {
        ...
        for (Int_t ich=0; ich < 96;ich++){
            if (LG_vs_ToT[FEBnumbers.at(ih)][ich]->GetEntries()>1) {
                ...
                TF1 *f = new TF1("f",bestfit,10,LastBin-2,4);
                //f->SetParLimits(0,0.,1e12);
                //f->SetParLimits(1,0.,1e12);
                f->SetParLimits(2,0.,1e12);

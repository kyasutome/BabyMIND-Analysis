...
for (vector<string>::iterator itFileName=vFileNames.begin(); itFileName != vFileNames.end(); itFileName++) {
 ...
 // return 0;
  ...
  ifs.close();
    ...
    Double_t rangePeaksMin = 50;
    Double_t rangePeaksMax = 700;
    ...
    for (Int_t iCh=0; iCh<96; iCh++) {
      ...
      Int_t nfound = s->Search(hFEBCH[iCh],peakWidth,"new",0.00000003);
	...
	for (vector<Peak>::iterator it = vPeaks.begin(); it != vPeaks.end(); it++ ) {
	  peak = *it;
	  if (peak.Height>100 && peak.Mean>rangePeaksMin && peak.Mean<rangePeaksMax ) {
